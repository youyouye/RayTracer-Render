...
void RayTracer::trace(Ray& ray, int depth, Color& color){
	...
	BRDF* brdf = in.primitive->getBRDF(in.localGeo);
	if (typeid(*brdf) == typeid(TextureBRDF))
	{
		color = (dynamic_cast<TextureBRDF*>(brdf)->texture->getColor(in));
	}
...
Color* RayTracer::shading(LocalGeo local, BRDF brdf, Ray lray, Color lcolor){
	Vector3 eyedirn = (eye - Vector3(local.pos)).normalize();
